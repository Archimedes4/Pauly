...
export default function GovermentRoomsCreate() {
  ...
  const [createRoomState, setCreateRoomState] = useState<loadingStateEnum>(loadingStateEnum.notStarted)
  const navigate = useNavigate();
  const [roomName, setRoomName] = useState<string>("")

  async function createRoom() {
    if (roomName !== "" && (createRoomState === loadingStateEnum.notStarted || createRoomState === loadingStateEnum.failed || createRoomState === loadingStateEnum.success)) {
  ...
  return (
    <View style={{width: width, height: height, backgroundColor: Colors.white}}>
      ...
      <View style={{flexDirection: "row"}}>
        ...
        <TextInput value={roomName} onChangeText={(e) => {setRoomName(e)}} placeholder='Room Name'/>
      </View>
      <Pressable onPress={() => {createRoom()}}>
        <Text>{(createRoomState === loadingStateEnum.notStarted) ? "Create Room":(createRoomState === loadingStateEnum.loading) ? "Loading":(createRoomState === loadingStateEnum.success) ? "Room Created":"Failed to Create Room" }</Text>
      </Pressable>
    </View>
  )
}