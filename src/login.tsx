...
export default function Login({onGetAuthToken, width}:{onGetAuthToken: () => void, width: number}) {
  ...
  return (
    <View style={{backgroundColor: "#793033", alignContent: "center", alignItems: "center", justifyContent: "center", height: height, width: width, overflow: "hidden"}}>
      <View style={{height: (width/(height * 0.5)) * 155, width: ((width/(height * 0.5)) * 175) * 2.2, flexDirection: "row"}} id='text outlinewr'>
        <Image source={require("../assets/images/PaulyLogo.png")} resizeMode='contain' style={{width: ((width/(height * 0.5)) * 175), height: ((width/(height * 0.5)) * 175)}} />
        <Text style={{position: "absolute", left: (width/(height * 0.5)) * 100, top: (30 * width/300),  fontFamily: "Gochi Hand", fontSize: ((width/(height * 0.5)) * 150), textShadowColor: 'rgba(0, 0, 0, 1)', textShadowOffset: {width: 1, height: 1}, textShadowRadius: 10, color: "white", pointerEvents: "none"}}>auly</Text>
      </View>
