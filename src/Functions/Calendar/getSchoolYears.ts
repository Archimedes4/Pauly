...
export default async function getSchoolYears(nextLink?: string): Promise<{ result: loadingStateEnum; events?: eventType[]; nextLink?: string}> {
  const result = await callMsGraph(`https://graph.microsoft.com/v1.0/groups/${orgWideGroupID}/calendar/events?&$filter=${store.getState().paulyList.eventTypeExtensionId}/eventType%20eq%20schoolYear`, "GET", true)
  if (result.ok){
