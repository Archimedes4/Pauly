...
async function getTimetablesFromSchoolYears(schoolYearIds: Map<string, number>, schedules: Map<string, scheduleType>): Promise<{result: loadingStateEnum, data?: Map<string, timetableType>}> {
  ...
  for (let schoolYearIndex = 0; schoolYearIndex < batchRequestResultSchoolYear.data.length; schoolYearIndex++) {
  ...
  for (let responseIndex = 0; responseIndex < batchRequestResultTimetable.data.length; responseIndex++) {
  ...
  for (let dressCodeIndex = 0; dressCodeIndex < batchRequestResultDressCode.data.length; dressCodeIndex++) {
  ...
  for (let timetableIndex = 0; timetableIndex < batchRequestResultTimetable.data.length; timetableIndex++) {
    if (batchRequestResultTimetable.data[timetableIndex].status === 200) {
      if (batchRequestResultTimetable.data[timetableIndex].body["value"].length === 1) {
        ...
        let timetableSchedules: scheduleType[] =[]
        const scheduleIds: string[] = JSON.parse(timetableData["timetableDataSchedules"])

        for (let scheduleIndex = 0; scheduleIndex < scheduleIds.length; scheduleIndex++) {
