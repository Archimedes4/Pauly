...
export default async function getPoints(): Promise<{result: loadingStateEnum, data?: number}> {
  let nextUrl =  `https://graph.microsoft.com/v1.0/sites/${store.getState().paulyList.siteId}/lists/${store.getState().paulyList.commissionSubmissionsListId}/items?expand=fields&$filter=fields/userId%20eq%20'${store.getState().microsoftProfileData.id}'%20and%20fields/submissionApproved%20eq%20true`
  let points: number = 0
  while (nextUrl !== "") {
    ...
    for (let index = 0; index < submissionResultClaimedData["value"]; index++){
