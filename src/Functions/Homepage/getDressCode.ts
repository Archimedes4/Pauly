...
export default async function getDressCode(dressCodeId: string): Promise<{result: loadingStateEnum, data?: dressCodeType}> {
  const result = await callMsGraph(`https://graph.microsoft.com/v1.0/sites/${store.getState().paulyList.siteId}/lists/${store.getState().paulyList.dressCodeListId}/items?expand=fields($select=dressCodeData,dressCodeIncentivesData,dressCodeName,dressCodeId)&$select=id&$filter=fields/dressCodeId%20eq%20'${dressCodeId}'`)
  if (result.ok) {
    const data = await result.json()
    if (data["value"].length !== 1){return {result: loadingStateEnum.failed}}
    try {
