...
export default async function getCurrentPaulyData() {
  ...
  if (result.ok){
    ...
    if (data["animationSpeed"] !== undefined && data["message"] !== undefined && data["powerpointId"] !== undefined) {
      const fileResult = await callMsGraph(`https://graph.microsoft.com/v1.0/shares/${data["powerpointId"]}/driveItem/content?format=pdf`, "GET", undefined, [{key: "Prefer", value: "redeemSharingLink"}])//`https://graph.microsoft.com/v1.0/shares/${data["powerpointId"]}/root?$select=@microsoft.graph.downloadUrl`
      if (fileResult.ok){
