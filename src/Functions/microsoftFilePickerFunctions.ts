...
export async function getUserMicrosoftFiles(path: string): Promise<{result: loadingStateEnum, data?: microsoftFileType[]}> {
  ...
  if (result.ok){
    ...
    for(let index = 0; index < data['value'].length; index += 1){
      newFiles.push({
        ...
        folder: (data['value'][index]['folder'] === undefined) ? false:true,
        ...
        type: (data['value'][index]['folder'] === undefined) ? data['value'][index]['file']['mimeType']:'folder'
      });
    };
...
export async function getUserTeams(): Promise<{result: loadingStateEnum.success, data: teamsGroupType[]} | {result: loadingStateEnum.failed}> {
  const result = await callMsGraph('https://graph.microsoft.com/v1.0/me/joinedTeams')//TODO make sure this works on live tenancy
  if (result.ok){
    const data = await result.json()
