...
export default function SegmentedPicker({width, height, selectedIndex, setSelectedIndex, options}:PickerWrapperProps) {
      ...
      <View style={{flexDirection: 'row', height: height, width: width, backgroundColor: "#7d7d7d", borderRadius: 20, shadowColor: '#000', shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.8, shadowRadius: 1}}>
        { options.map((option, index) => (
          <Pressable onPress={() => {
            ...
            <Text adjustsFontSizeToFit={true} style={{textAlignVertical: "center", textAlign: "center", width: compoentWidth}}>{option}</Text>
          </Pressable>
        ))}
