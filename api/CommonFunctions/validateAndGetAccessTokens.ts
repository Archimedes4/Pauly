...
export default async function validateAndGetAccessTokens(context: Context, req: HttpRequest): Promise<validationReturn> {
    //On behalf of flow
    try {
        ...
        const authResult = await fetch("https://login.microsoftonline.com/" + config. tenantId +"/oauth2/v2.0/token", {
            method: "POST",
            body: authPostData,
            headers: {"Content-Type":"application/x-www-form-urlencoded"}
        })
        if (!authResult.ok) {context.res = {status: 401,body: "Unauthorized: something went wrong validating token"}; return}
        ...
        const clientCredentialsResult = await fetch("https://login.microsoftonline.com/" + config.tenantId +"/oauth2/v2.0/token", {
            method: "POST",
            body: postData,
            headers: {"Content-Type":"application/x-www-form-urlencoded"}
        })
        ...
        context.res = {status: 401,body: "Unauthorized: something went wrong validating token"}
    }
}
