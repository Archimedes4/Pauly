...
function isListResponse(response: listResponce | HttpResponseInit): response is listResponce {
  return (response as listResponce).siteId !== undefined;
}

export async function ClaimCommission(req: HttpRequest, context: InvocationContext): Promise<HttpResponseInit> {
  try {
    const accessTokens = await validateAndGetAccessTokens(context, req)
    const paulyListResult = await getPaulyList(accessTokens.onBehalfOfAccessToken)
    
    if (!isListResponse(paulyListResult)) {return paulyListResult;};
    
    if (paulyListResult === undefined || accessTokens === undefined) {return {status: 400, body: "Bad Request"};}

    const requestBody = await req.json()

    const commissionId = (req.query.get('commissionId') || (requestBody['commissionId']));
    if (commissionId === undefined || commissionId === "") {return {status: 400, body: "Error: The CommissionId needs to be supplied"};}
    if (typeof commissionId !== "string") {return {status: 400, body: "Error: The CommissionId needs to be a string"};}
    ...
    if (!commissionResult.ok){return {status: 500, body: "Internal Error: Unable To Get Commission (This doesn't mean that the commission does not exist)"};}
    ...
    if (commissionResultData["value"].length !== 1){
      if (commissionResultData["value"].length !== 0) {
        return {status: 400, body: "Error: The CommissionId supplied doesn't have any asociated commissions attached to it"};
      } else {
        return {status: 500, body: "Internal Error: The CommissionId supplied has mutiple commissions. Please submit a bug report"};
      }
    }
``
    ...
    if (value === 2 || value === 4) {
      const latCoordinate = (req.query.get('latCoordinate') || (req.body && requestBody['latCoordinate']));
      if (latCoordinate === undefined || latCoordinate === "") {return {status: 400, body: "Error: A lat coordinaten needs to be supplied"};}
      const lngCoordinate = (req.query.get('lngCoordinate') || (req.body && requestBody['lngCoordinate']));
      if (lngCoordinate === undefined || lngCoordinate === "") {return {status: 400, body: "Error: A lng coordinaten needs to be supplied"};}
      resultSubmissionData["location"]["lat"] = latCoordinate
      resultSubmissionData["location"]["lng"] = lngCoordinate
    }
    if (value === 3 || value === 4) {
      const imageShare: string = (req.query.get('imageShare') || (req.body && requestBody['imageShare']));
      if (imageShare === undefined || imageShare === "") {return {status: 400, body: "Error: The ImageShare needs to be supplied"};}
    ...
    if (!getUserResult.ok) {return {status: 500, body: "Internal Error: Could Not Get User"}; return}
    ...
    while (nextUrl !== ""){
      ...
      if (!submissionResultClaimed.ok) {return {status: 500, body: "Internal Error: Could Not Get Submissions"}; return}
    ...
    if (reachedMaxNumberOfClaims) {return {status: 400, body: "Reached max number of claims"}; return}
    ...
    if (!submissionResultUnClaimed.ok) {return {status: 500, body: "Internal Error: Could Not Get Submissions"}; return}

    if (submissionResultUnclaimedData["value"] !== undefined) {
      if (submissionResultUnclaimedData["value"].count >= 1) {
        if (!allowMultipleSubmissions) {return {status: 400, body: "Reached max number of submissions"}; return}
      }
    }
    ...
    if (!submitResult.ok) {return {status: 500, body: "Internal Error: Unable to create submission"}; return}
